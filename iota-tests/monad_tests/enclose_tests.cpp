...
TEST_CASE("enclose integer", "[monad]")
{
  ...
  REQUIRE(evalExpression({5, enclose}) == a{5});
}
...
TEST_CASE("enclose real", "[monad]")
{
  ...
  REQUIRE(evalExpression({5.0f, enclose}) == a{5.0f});
}
...
TEST_CASE("enclose list", "[monad]")
{
  ...
  REQUIRE(evalExpression({{0, 1, 2}, enclose}) == a{{0, 1, 2}});
  REQUIRE(evalExpression({{0.0f, 1.0f, 2.0f}, enclose}) == a{{0.0f, 1.0f, 2.0f}});
//  FIXME
